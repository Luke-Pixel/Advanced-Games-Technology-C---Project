...
void main()
{
	...
	if(num_bones>0){
		...
		if (Weights[0]>0.0f)
		{
			...
			BoneTransform = gBones[BoneIDs[0]] * 0.0001;
		
		if (WeightsTotal < 0.999f)
		{
			if (Weights[1]>0.0f)
			{
				WeightsTotal += Weights[1];
				BoneTransform     += gBones[BoneIDs[1]] * Weights[1];
			}
			//else
			//	BoneTransform     += gBones[BoneIDs[1]] * 0.0001;
				
			if (WeightsTotal < 0.999f)
			{
				if (Weights[2]>0.0f)
				{
					WeightsTotal += Weights[2];
					BoneTransform     += gBones[BoneIDs[2]] * Weights[2];
				}
				//else
				//	BoneTransform     += gBones[BoneIDs[2]] * 0.0001;
				...
				//	else
				//		BoneTransform     += gBones[BoneIDs[3]] * 0.0001;
				}
			}
		}
