...
end_game::end_game(const std::string& path, float max_time, float width, float height)
{
	m_max_time = max_time;
...
void end_game::on_update(const engine::timestep& time_step)
{
	if (!s_active)
		return;
...
void end_game::on_render(engine::ref<engine::shader> shader)
{
	if (!s_active)
		return;
...
engine::ref<end_game> end_game::create(const std::string& path, float max_time, float width, float height)
{
	return std::make_shared<end_game>(path, max_time, width, height);
}
